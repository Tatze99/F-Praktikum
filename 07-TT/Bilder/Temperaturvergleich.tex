\begin{wrapfigure}{r}{6.1cm}
  \vspace{-.5cm}
  \centering
  \begin{tikzpicture}
    \begin{axis}[width=5.9cm, height=6.5cm, xlabel=Zeit $t$ in \si{\minute}, xmin=100, xmax=140, ymin=110, ymax=160, ylabel={$T$ in \si{\kelvin}}, legend cell align={left}, xtick scale label code/.code={}]
      \addplot[blue] table[x index=0,y index=1]{Daten/Halbleitermessung_Zeitverlauf.txt};
      \addplot[orange ] table[x index=0,y index=2]{Daten/Halbleitermessung_Zeitverlauf.txt};
      \addplot[green!50!black] table[x index=0,y index=3]{Daten/Halbleitermessung_Zeitverlauf.txt};
      \legend{Pt 100, Pt 1000, Diode};
    \end{axis}
  \end{tikzpicture}
  \caption{Temperaturkurven verschie\-de\-ner Mess\-widerstände.}
  \label{fig:Temperaturvergleich}
\end{wrapfigure}
