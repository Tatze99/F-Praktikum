...
\begin{figure}[htp]
    \centering
        \includegraphics[width=0.8\textwidth]{Bilder/McMahanMaschine.pdf}
...
\paragraph{Sieden von kryogenen Flüssigkeiten}
...
So kann durch Plotten des Logarithmus naturales der Temperaturdifferenz $\Delta T$ bzw. der Temperaturdifferenz verringert um die Ausgleichstemperatur und Bestimmung des Anstieges mit bekanntem $R_{th}$ die Wärmekapazität bestimmt werden. \\
...
\subsubsection{mittels Abkühlkurve}
...
\begin{table}[htp]
  ...
  \label{tab:CV}
  \begin{tabular}{l l l 1 1 c}
    \toprule
     & Cu 1. Ber & Cu 2. Ber & Pb 1. Ber & Pb 2. Ber\\
    \midrule
    Temperaturbereich & \SI{80,29}{\kelvin}-\SI{77,1}{\kelvin} & \SI{37,19}{\kelvin}-\SI{29,98}{\kelvin}&\SI{90,02}{\kelvin}-\SI{87,10}{\kelvin} & \SI{41,71}{\kelvin} - \SI{37,06}{\kelvin}
    \\
    Anstieg b & 0,001317&0,005993167&0,0009285
    &0,000770333
    \\
    $\dot{Q}$ & 2,1504 &0,4784& 2,1504&0,4784
    \\
    $\Delta T_0$ & 69,8 &	34,8	&70,39711&	33,68231
    \\
    $R_th$&32,45907738 & 72,74247492 & 32,7367513	&70,40616639
    \\
    $C_v $ & 23,39257625 &	2,293800123	& 32,89898602	& 18,43786418
    \\
    \bottomrule
  \end{tabular}
\end{table}
...
\subsubsection{mittels Dual-Slope-Methode}
Eine andere Methode zur Bestimmung der Wärmekapazität wird in Abschnitt \ref{sec:Dual-Slope} beschrieben. Sie beruht auf dem Paper %PAPER EINFÜGEN
An der dort vorgestellten Methode muss lediglich berücksichtigt werden, dass wir im Bereich der \SI{400}{\milli\volt} auch im Kühlbereich weiterhin eine Heizleistung haben. Dieser Umstand ist bei der in Gleichung \ref{equ:DS} bereits berücksichtigt. Für den Bereich der \SI{0}{\milli\ampere} wird die zweite Heizleistung Null gesetzt. Wird dies beachtet, ist keine weitere Messung für die Bestimmung der Wärmekapazität mit diesem Verfahren im Vergleich zum vorherigen nötig. Zur besseren Vergleichbarkeit der Ergebnisse werden die gleichen Temperaturbereiche betrachtet. \\
...
\begin{figure}[ht!]
  ...
  \begin{subfigure}[b!]{0.45\textwidth}
    ...
    \caption{Blei, T=\SI{41,71}{\kelvin} bis \SI{37,06}{\kelvin}} \end{subfigure}
  \caption{zur Ermittlung der Wärmekapazität mit Dual-Slope-Methode bestimmte Anstiege der Temperatur}
  \label{fig:DS_LineareFits}
\end{figure}

\FloatBarrier
Mit den so erhaltenen Werten lässt sich jetzt nach Gleichung \ref{equ:DS} die Wärmekapazität berechnen. Die dafür nötigen Werte sowie die Ergebnisse sind in \ref{tab:ergebnisseDS} zusammengefasst.


\begin{tabular}{l l l 1 1 c}\label{tab:ergebnisseDS}
  \toprule
   & Cu 1. Ber & Cu 2. Ber & Pb 1. Ber & Pb 2. Ber\\
  \midrule
  Temperaturbereich & T=\SI{77,02}{\kelvin} bis \SI{80,17}{\kelvin} & \\SI{28,71}{\kelvin} bis \SI{36,97}{\kelvin} & T=\SI{87,13}{\kelvin} bis \SI{90,06}{\kelvin} & \SI{37.06}{\kelvin} bis \SI{41,79}\\
  & T=\SI{80,29}{\kelvin} bis \SI{77,10}{\kelvin} & T=\SI{37,19}{\kelvin} bis \SI{28,98}{\kelvin} & T=\SI{90,02}{\kelvin} bis \SI{87,10}{\kelvin} &T=\SI{41,71}{\kelvin} bis \SI{37,06}{\kelvin}\\
  $\dot{T}_C$ &-0,042302833	&-0,125922	&-0,038845	& -0,022994333\\
  $\dot{T}_H$ & 0,0446835 &	0,5490845	&0,034475&	0,1044335\\

  $C_v$ & 19,22141026	& 3,1857471	& 22,80414621	& 16,87543407\\

  Referenz&12,4-13,0 &	1,6-3,1	&23,7-23,8	&19,1-20,2\\
  \bottomrule
\end{tabular}
\end{table}
